...
export default function Home(){
  ...
  const handlePay = async (packageId: number) => {
    const res = await fetch("/api/create-checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ playerId, packageId }) });
  ...
  return (<div className="bg-black text-white min-h-screen p-10 text-center">
    ...
    <input className="mt-5 p-2 text-black" placeholder="ID اللاعب" onChange={(e)=>setPlayerId(e.target.value)}/>
